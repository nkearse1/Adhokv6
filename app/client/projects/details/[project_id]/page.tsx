...
export default function ClientProjectDetail() {
  ...
  useEffect(() => {
    async function loadBids() {
      try {
        ...
        const json = await res.json();
        const list = (json.data || []).filter((b: any) => b.projectId === project_id);
        setBids(list);
      } catch (err) {
  ...
  const projectHasAcceptBid = project.metadata?.acceptBidEnabled;
  const acceptBidEnabled = accountHasAcceptBid || projectHasAcceptBid;
    ...
    <div className="p-4 sm:p-6 max-w-4xl mx-auto">
      <Tabs defaultValue="details" className="space-y-6">
                ...
                {talentProfile && (
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      <span className={`inline-flex items-center gap-2 px-3 py-1 text-sm font-semibold rounded-full ${getBadgeStyle(
        ...
        <TabsContent value="bids" className="space-y-4">
          {acceptBidEnabled && (
            <Badge variant="success" className="w-max">Accept Bid Enabled</Badge>
          )}
          {bids.length === 0 ? (
            ...
            <div className="space-y-3">
              {bids.map((bid: any) => (
                ...
                >
                  <div>
                    <p className="font-semibold">{bid.professionalId}</p>
      ...
      <AcceptBidUpsellModal
        ...
        onEnabled={() =>
          setProject({
            ...project,
            metadata: { ...(project.metadata || {}), acceptBidEnabled: true },
          })
        }
      />
    </div>
