...
export default function ActiveBidsPanel({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const userBids = (json.data || []).filter((b: any) => b.professionalId === userId);
        setBids(userBids);
        const projectIds = userBids.map((b: any) => b.projectId);
        if (projectIds.length) {
          const pres = await fetch('/api/db?table=projects');
          const pjson = await pres.json();
          setProjects((pjson.data || []).filter((p: any) => projectIds.includes(p.id)));
        }
      } catch (err) {
  ...
  if (!bids.length) {
    return <div className="border p-4 rounded-md text-muted-foreground">No active bids.</div>;
  }
